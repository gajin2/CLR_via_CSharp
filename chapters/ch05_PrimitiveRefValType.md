...
2. C# 的 `long` 映射到 `System.Int64` ，但在其他编程语言中，`long` 可能映射到 `Int16` 或 `Int32`。例如， C++/CLI 就将 `long` 视为 `Int32`。习惯于用一种语言写程序的人在看用另一种语言写程序的人写的源代码时，很容易错误理解代码意图。事实上，大多数语言甚至不将 `long` 当作关键字，根本不编译使用了它的代码。
...
不同语言处理溢出的方式不同。C 和 C++ 不将溢出视为错误，允许值*回滚(wrap)*；应用程序将「若无其事」地运行。相反， Microsoft Visual Basic 总是将溢出视为错误，并在检测到溢出时抛出异常。
...
检查同一性(看两个引用是否指向同一个对象)务必调用 `ReferenceEquals`，不应使用 C# 的 == 操作符(除非先把两个操作数都转型为 `Object`)，因为某个操作数的类型可能重载了 == 操作符，为其赋予不同于「同一性」的语义。
...
在内部，`ValueType`的`Equals`方法利用反射(详情将在第 23 章「程序集加载和反射」讲述)完成上述步骤3。由于 CLR 反射机制慢，定义自己的值类型时应重写`Equals`方法来提供自己的实现，从而提供用自己类型的实例进行值相等性比较的性能。当然，自己的实现不调用 `base.Equals`。
...
以下示例 C# 代码使用 COM `IDispatch` 创建 Microsoft Office Excel 工作薄，将一个字符串放到单元格 A1 中：
...
可利用 C# 的`dynamic` 类型重写上述代码，并使用获得了显著简化的语法：
